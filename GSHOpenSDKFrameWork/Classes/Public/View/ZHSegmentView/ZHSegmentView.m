...
static NSString *const cellIdentifier = @"zhSegmentViewItemCell";

@interface ZHSegmentView () <UICollectionViewDelegateFlowLayout,UICollectionViewDataSource>
...
-(UICollectionView*)collectionView {
    if (!_collectionView) {
        ...
        [_collectionView registerClass:[ZHSegmentViewItemCell class] forCellWithReuseIdentifier:cellIdentifier];
...
- (void)setContentOffsetWithCellFrame:(CGRect)frame {
    ...
    CGRect centerRect = CGRectMake(CGRectGetMidX(frame) - CGRectGetWidth(self.collectionView.bounds)/2, 0,CGRectGetWidth(self.collectionView.bounds), CGRectGetHeight(self.collectionView.bounds));
    NSLog(@"rect x : %f",centerRect.origin.x);
    [self.collectionView scrollRectToVisible:centerRect animated:YES];
//    return;
//    CGFloat centX = CGRectGetWidth(self.collectionView.frame)/2;
//    CGFloat offsetX = 0;
//    if (CGRectGetMidX(frame) <= centX) {
//        offsetX = 0;
//    } else if (CGRectGetMidX(frame) + centX >= self.collectionView.contentSize.width) {
...
- (void)resizeLineViewWihtCellFrame:(CGRect)frame animated:(BOOL)animated {
    CGFloat height = 3.0f;
    CGRect rect = CGRectMake(CGRectGetMinX(frame)+(frame.size.width / 2.0 - 12),
                             CGRectGetHeight(self.collectionView.frame)-height-_lineEdgeInsets.bottom,
                             24, height-_lineEdgeInsets.top);
    if (animated) {
...
- (void)selectItemAtIndexPath:(NSIndexPath*)indexPath {
    ZHSegmentViewItemCell *cell = (ZHSegmentViewItemCell*)[self.collectionView cellForItemAtIndexPath:indexPath];
    cell.selected = YES;
...
{
    ZHSegmentViewItemCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:cellIdentifier forIndexPath:indexPath];
    ...
    if (collectionView.indexPathsForSelectedItems.count <= 0 && indexPath.item == _currentIndex) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    self.segmentItemClickBlock(indexPath.item);
    [self selectItemAtIndexPath:indexPath];
}

- (void)collectionView:(UICollectionView *)collectionView didDeselectItemAtIndexPath:(NSIndexPath *)indexPath {
    ZHSegmentViewItemCell *cell = (ZHSegmentViewItemCell*)[collectionView cellForItemAtIndexPath:indexPath];
    cell.selected = NO;
}
...
@interface ZHSegmentViewItemCell ()

@property (nonatomic, strong) UILabel *titleLabel;

@end

@implementation ZHSegmentViewItemCell
