...
-(void)exchangeCurrentView:(NSInteger)old new:(NSInteger)new
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        if(new<old)
            lx+=strongSelf.controlSize *(strongSelf.currentMultiple - strongSelf.otherMultiple);
        oldSelect.frame=CGRectMake(lx, mpSelect.origin.y, strongSelf.controlSize * strongSelf.otherMultiple, strongSelf.controlSize);
        
        CGFloat mx=newTemp.origin.x;
        if(new>old)
            mx-=strongSelf.controlSize * (strongSelf.currentMultiple - strongSelf.otherMultiple);
        newSeltect.frame=CGRectMake(mx, newTemp.origin.y, strongSelf.controlSize * strongSelf.currentMultiple, strongSelf.controlSize);
        ...
        if(new-old>1)
        {
            for(NSInteger t=old+1;t<new;t++)
            {
                ...
                ms.frame=CGRectMake(ms.frame.origin.x-strongSelf.controlSize * (strongSelf.currentMultiple - strongSelf.otherMultiple), ms.frame.origin.y, strongSelf.controlSize*strongSelf.otherMultiple, strongSelf.controlSize);
            }
        }
        // 右边选中到左边的时候 越过点击
        if(new-old<-1)
